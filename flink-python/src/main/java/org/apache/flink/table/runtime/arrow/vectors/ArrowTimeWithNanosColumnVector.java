...
/** ArrowTimeWithNanosColumnVector. */
public class ArrowTimeWithNanosColumnVector implements LongColumnVector {

    /** Container which is used to store the sequence of time values of a column to read. */
    private final ValueVector valueVector;

    public ArrowTimeWithNanosColumnVector(ValueVector valueVector) {
    ...
    public long getLong(int i) {
        if (valueVector instanceof TimeSecVector) {
            return ((TimeSecVector) valueVector).get(i) * 1000_000_000L;
        } else if (valueVector instanceof TimeMilliVector) {
            return ((TimeMilliVector) valueVector).get(i) * 1000_000L;
        } else if (valueVector instanceof TimeMicroVector) {
            return (((TimeMicroVector) valueVector).get(i) * 1000);
        } else {
            return ((TimeNanoVector) valueVector).get(i);
        }
    }
