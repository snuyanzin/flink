...
public class RelBuilder {
    ...
    private RelOptTable.ViewExpander getViewExpander(RelOptCluster cluster, Context context) {
        return Util.first(
                context.unwrap(RelOptTable.ViewExpander.class),
    ...
    private Config getConfig(Context context) {
        final Config config = Util.first(context.unwrap(Config.class), Config.DEFAULT);
    ...
    public RexNode literal(Object value) {
        ...
        if (value == null) {
            ...
            return rexBuilder.makeLiteral(
                    value, getTypeFactory().createSqlType(SqlTypeName.SYMBOL), false);
        } else {
    ...
    /** Creates a call to a scalar operator. */
    public @Nonnull RexNode call(SqlOperator operator, Iterable<? extends RexNode> operands) {
        return call(operator, ImmutableList.copyOf(operands));
    }
     ...
     */
    public GroupKey groupKey(
            ImmutableBitSet groupSet, @Nonnull Iterable<? extends ImmutableBitSet> groupSets) {
    ...
    @Deprecated // to be removed before 2.0
    public GroupKey groupKey(
            ImmutableBitSet groupSet, boolean indicator, ImmutableList<ImmutableBitSet> groupSets) {
        Aggregate.checkIndicator(indicator);
        return groupKey_(
                groupSet,
                groupSets == null ? ImmutableList.of(groupSet) : ImmutableList.copyOf(groupSets));
    ...
    private GroupKey groupKey_(
            ImmutableBitSet groupSet, @Nonnull ImmutableList<ImmutableBitSet> groupSets) {
        ...
        if (!force && RexUtil.isIdentity(nodeList, input.getRowType())) {
            ...
            if (input instanceof Values && fieldNames != null) {
                ...
                Pair.forEach(
                        ...
                        (name, field) -> typeBuilder.add(name, field.getType()));
    ...
    public RelBuilder aggregate(GroupKey groupKey, List<AggregateCall> aggregateCalls) {
        return aggregate(
                groupKey,
                aggregateCalls.stream().map(AggCallImpl2::new).collect(Collectors.toList()));
    }
    ...
    public RelBuilder aggregate(GroupKey groupKey, Iterable<AggCall> aggCalls) {
        ...
        if (groupKey_.nodeLists != null) {
            ...
            if (Iterables.any(aggCalls, RelBuilder::isGroupId)) {
                return rewriteAggregateWithGroupId(
            ...
            if (registrar.extraNodes.size() > sizeBefore) {
                throw new IllegalArgumentException(
                        "group sets contained expressions not in group key: "
                                + registrar.extraNodes.subList(
                                        sizeBefore, registrar.extraNodes.size()));
            }
        } else {
     ...
     * function in runtime.
     *
     * <p>To fill this gap, an aggregation query that contains {@code GROUP_ID()} function will
     * generally be rewritten into UNION when converting to RelNode.
     *
     * <p>Also see the discussion in <a
     * href="https://issues.apache.org/jira/browse/CALCITE-1824">[CALCITE-1824] GROUP_ID returns
     * wrong result</a>.
     */
    private RelBuilder rewriteAggregateWithGroupId(
            ImmutableBitSet groupSet,
        ...
        for (int groupId = 0; groupId <= maxGroupId; groupId++) {
            ...
            stack.push(frame);
            aggregate(
                    groupKey(groupSet, groupIdToGroupSets.get(groupId)),
    ...
    private RelBuilder setOp(boolean all, SqlKind kind, int n) {
        ...
        if (config.simplifyValues()
            ...
            RelDataType rowType =
                    getTypeFactory().leastRestrictive(Util.transform(inputs, RelNode::getRowType));
    ...
    private class AggCallImpl implements AggCallPlus {
        ...
        public AggregateCall aggregateCall(
            ...
            final List<Integer> args = registrar.registerExpressions(this.operands);
            final int filterArg =
     ...
     *
     * @deprecated Use the {@code withXxx} methods in {@link RelBuilder.Config}.
     */
    @Deprecated // to be removed before 2.0
    public static class ConfigBuilder {
        ...
        /**
         * Sets the value that will become {@link
         * org.apache.calcite.tools.RelBuilder.Config#dedupAggregateCalls}.
         */
        ...
        /**
         * Sets the value that will become {@link
         * org.apache.calcite.tools.RelBuilder.Config#simplify}.
         */
