...
public class RexInterpreter implements RexVisitor<Comparable> {
    ...
    /**
     * Translates the values in a RangeSet from literal format to runtime format. For example the
     * DATE SQL type uses DateString for literals and Integer at runtime.
     */
    @SuppressWarnings({"BetaApi", "rawtypes", "unchecked", "UnstableApiUsage"})
    ...
    private static Comparable coalesce(RexCall call, List<Comparable> values) {
        for (Comparable value : values) {
            if (value != N) {
                return coerceToType(value, call.getType());
            }
        }
        return N;
    }

    private static Comparable coerceToType(Comparable value, RelDataType targetType) {
        if (value == N) {
            return N;
        }
        switch (targetType.getSqlTypeName()) {
            case DECIMAL:
                if (value instanceof BigDecimal) {
                    ...
                    if (targetScale != RelDataType.SCALE_NOT_SPECIFIED
                        ...
                        return bd.setScale(targetScale, RoundingMode.HALF_UP);
                    }
                }
                return value;
            case DOUBLE:
            ...
            case VARCHAR:
                if (value instanceof NlsString) {
                    // CHAR(n) is space-padded, VARCHAR(m) is not — trim trailing spaces on
                    // widening.
                    ...
                    if (end != v.length()) {
                        return new NlsString(
                                v.substring(0, end), s.getCharsetName(), s.getCollation());
                    }
                }
                return value;
            default:
