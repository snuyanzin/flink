 ...
 * CALCITE-4554.
 *
 * <p>Lines 240 ~ 243, Flink improves error message for functions without appropriate arguments in
 * handleUnresolvedFunction.
 *
 * <p>Lines 2127 ~ 2141, Flink improves error message for functions without appropriate arguments in
 * handleUnresolvedFunction at {@link SqlValidatorImpl#handleUnresolvedFunction}.
 *
 * <p>Lines 2396 ~ 2407
 *
 * <p>Lines 2563 ~ 2565, CALCITE-7471 should be removed after upgrading Calcite to 1.42.0.
 *
 * <p>Lines 2679 ~ 2698, CALCITE-7217, CALCITE-7312 should be removed after upgrading Calcite to
 * 1.42.0.
 *
...
public class SqlValidatorImpl implements SqlValidatorWithHints {
    ...
    private boolean expandSelectItem(
        ...
        if (SqlValidatorUtil.isMeasure(selectItem)) {
            ...
            // Non-strict GROUP BY or BY clause: wrap non-aggregated, non-grouped columns in
            // ANY_VALUE()
            if (isAggregate(select)
                    && (config.conformance().isNonStrictGroupBy() || select.hasByClause())
                        ...
                        for (RelDataTypeField field : rowType.getFieldList()) {
                            ...
                            final SqlIdentifier exp =
                                    new SqlIdentifier(
                                            ...
                                            startPosition);
                            recordExcludeMatches(
                                    excludeIdentifiers, exp, nameMatcher, excludeMatched);
            ...
            default:
                ...
                if (rowType.isDynamicStruct()) {
                    ...
                    for (RelDataTypeField field : rowType.getFieldList()) {
                        ...
                        final SqlIdentifier columnId = prefixId.plus(columnName, startPosition);
                        recordExcludeMatches(
                                excludeIdentifiers, columnId, resolvedNameMatcher, excludeMatched);
    ...
    private static boolean matchesExcludeNames(
            ...
            List<String> excludedIdentifierNames,
    ...
    private static boolean shouldExcludeField(
            @Nullable SqlNodeList excludeList, SqlIdentifier columnId, SqlNameMatcher nameMatcher) {
        if (excludeList == null) {
            return false;
        }
    ...
    private static boolean matchesExcludeIdentifier(
            SqlIdentifier columnId, SqlIdentifier excludeIdentifier, SqlNameMatcher nameMatcher) {
        return matchesExcludeNames(columnId.names, excludeIdentifier.names, nameMatcher);
    }

    private static List<SqlIdentifier> extractExcludeIdentifiers(
            @Nullable SqlNodeList excludeList) {
        if (excludeList == null) {
    ...
    private static void recordExcludeMatches(
    ...
    private void throwIfUnknownExcludeColumns(
            List<SqlIdentifier> excludeIdentifiers, boolean[] excludeMatched) {
            ...
            if (sampleSpec instanceof SqlSampleSpec.SqlTableSampleSpec) {
                ...
                if (samplePercentage.compareTo(BigDecimal.ZERO) < 0
                    ...
                    throw SqlUtil.newContextException(
                            node.getParserPosition(), RESOURCE.invalidSampleSize());
                }
    ...
    protected void addToSelectList(
        ...
        ((PairList<String, RelDataType>) fieldList).add(uniqueAlias, deriveType(scope, exp));
        list.add(exp);
    }
     ...
     *
     * <p>{@code parentScope0} and {@code usingScope} are often the same. They differ when the
        ...
        switch (kind) {
            ...
            case JOIN:
                ...
                if (join.getJoinType() == JoinType.ASOF
                        || join.getJoinType() == JoinType.LEFT_ASOF) {
    ...
    private void registerQuery(
        ...
        switch (node.getKind()) {
            ...
            case UNION:
                registerSetop(parentScope, usingScope, node, enclosingNode, alias, forceNullable);
                break;
    ...
    private void registerSetop(
        ...
        for (int i = 0; i < call.getOperandList().size(); i++) {
            ...
            registerQuery(scope, null, operand, operand, null, false);
        }
    }
    ...
    private void registerWith(
        ...
        for (SqlNode withItem_ : with.withList) {
            ...
            registerQuery(
                    ...
                    null,
                    withItem.query,
                    withItem.recursive.booleanValue() ? withItem : with,
            ...
            if (insertCallAfterValidate.getSource() instanceof SqlSelect) {
                ...
                for (int i = 0; i < sourceSelectList.size(); i++) {
                    ...
                    if (!targetField.getType().isNullable()
                            && SqlUtil.isNullLiteral(selectItem, true)) {
                        throw newValidationError(
                                selectItem, RESOURCE.columnNotNullable(targetField.getName()));
                    }
                }
            }
        }
    ...
    private static class Expander extends SqlScopedShuttle {
        ...
        /**
         * Expands star (*) within ROW constructors. For example, transforms {@code ROW(*)} or
         * {@code ROW(t.*)} into {@code ROW(col1, col2, ...)} based on available columns in scope.
         *
        ...
        private SqlNode expandStarInRow(SqlNode node) {
            ...
            if (!(scope instanceof SelectScope)) {
                ...
                for (SqlNode operand : call.getOperandList()) {
                    if (operand instanceof SqlIdentifier && ((SqlIdentifier) operand).isStar()) {
                        throw validator.newValidationError(node, RESOURCE.rowStarNotAllowed());
                    }
                }
                return node;
            }
            ...
            return SqlStdOperatorTable.ROW.createCall(call.getParserPosition(), expandedOperands);
        }
