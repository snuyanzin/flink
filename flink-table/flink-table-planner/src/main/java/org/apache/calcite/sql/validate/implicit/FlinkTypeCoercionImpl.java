...
public class FlinkTypeCoercionImpl extends AbstractTypeCoercion {

    public FlinkTypeCoercionImpl(RelDataTypeFactory typeFactory, SqlValidator validator) {
        super(typeFactory, validator);
    }

    /**
     * Widen a SqlNode's field type to common type, mainly used for set operations like UNION,
     ...
     * </pre>
     *
     * For struct type (type1, type2, type3) union type (type4, type5, type6), infer the first
     * result column type type7 as the wider type of type1 and type4, the second column type as the
     * wider type of type2 and type5 and so on.
     *
        ...
        switch (kind) {
            ...
            case VALUES:
                for (SqlNode rowConstructor : ((SqlCall) query).getOperandList()) {
                    if (!coerceOperandType(
                            scope, (SqlCall) rowConstructor, columnIndex, targetType)) {
                        return false;
                    }
                }
            ...
            case WITH:
                ...
                return rowTypeCoercion(validator.getOverScope(body), body, columnIndex, targetType);
            case UNION:
            case INTERSECT:
            case EXCEPT:
     ...
     * to data type of the other operand if its data type is NUMERIC; If the other operand is
     * DECIMAL, coerce the STRING operand to max precision/scale DECIMAL.
     */
    @Override
     ...
     *       `0=false` all to be true; 2. If operands are datetime and string, do nothing because
     *       the SqlToRelConverter already makes the type coercion;
     *   <li>For binary comparison [=, &gt;, &gt;=, &lt;, &lt;=]: try to find the common type, i.e.
     *       "1 &gt; '1'" will be converted to "1 &gt; 1";
     *   <li>For BETWEEN operator, find the common comparison data type of all the operands, the
     *       common type is deduced from left to right, i.e. for expression "A between B and C",
     *       finds common comparison type D between A and B then common comparison type E between D
     *       and C as the final common type.
     * </ul>
     */
    @Override
    ...
    /**
     * Finds the common type for binary comparison when the size of operands {@code dataTypes} is
     * more than 2. If there are N(more than 2) operands, finds the common type between two operands
     * from left to right:
     *
     * <p>Rules:
     *
     ...
     * makes the conversion but we still keep this interface overridable so user can have their
    ...
    /**
     * Casts "BOOLEAN = NUMERIC" to "NUMERIC = NUMERIC". Expressions like 1=`expr` and 0=`expr` can
     * be simplified to `expr` and `not expr`, but this better happens in {@link
    ...
    /**
     * CASE and COALESCE type coercion, collect all the branches types including then operands and
     * else operands to find a common type, then cast the operands to the common type when needed.
     */
    @Override
    public boolean caseWhenCoercion(SqlCallBinding callBinding) {
        ...
        if (null != widerType) {
            ...
            if (needToCast(scope, elseOp, widerType)) {
                coerced = coerceOperandType(scope, caseCall, 3, widerType) || coerced;
            }
            return coerced;
        }
        return false;
     ...
     *       sub-query expression's row type. Wrap the fields of the LHS and RHS in CAST operators
     *       if it is needed.
            ...
            for (int i = 0; i < widenTypes.size(); i++) {
                ...
                if (node1.getKind() == SqlKind.ROW) {
                    ...
                    if (coerceOperandType(scope, (SqlCall) node1, i, desired)) {
                        updateInferredColumnType(
                                requireNonNull(scope, "scope"), node1, i, widenTypes.get(i));
                        coerced = true;
                    }
                } else {
                ...
                if (node2 instanceof SqlNodeList) {
                    ...
                    if (type2.isStruct()) {
                        for (SqlNode node : (SqlNodeList) node2) {
                            ...
                            listCoerced =
                                    coerceOperandType(scope, (SqlCall) node, i, desired)
                                            || listCoerced;
                        }
                        if (listCoerced) {
                            updateInferredColumnType(
                                    requireNonNull(scope, "scope"), node2, i, desired);
                        }
                    } else {
        ...
        for (int i = 0; i < operandTypes.size(); i++) {
            ...
            coerced =
                    null != implicitType
                                    ...
                                    && coerceOperandType(
                                            binding.getScope(), binding.getCall(), i, implicitType)
                            || coerced;
        }
        return coerced;
    }
    ...
    public boolean userDefinedFunctionCoercion(
        ...
        for (int i = 0; i < call.operandCount(); i++) {
            ...
            if (operand.getKind() == SqlKind.ARGUMENT_ASSIGNMENT) {
                ...
                coerced =
                        coerceOperandType(scope, (SqlCall) operand, 0, paramTypes.get(formalIndex))
                                || coerced;
            } else {
    ...
    public boolean querySourceCoercion(
        ...
        for (int i = 0; i < sourceCount; i++) {
            ...
            if (!SqlTypeUtil.equalSansNullability(
                            validator.getTypeFactory(), sourceType, targetType)
    ...
    /**
     * Coerces the field expression at index {@code columnIndex} of source in an INSERT or UPDATE
     * query to target type.
     *
    ...
    private boolean coerceSourceRowType(
        ...
        switch (query.getKind()) {
            case INSERT:
                ...
                return coerceSourceRowType(
                        sourceScope, insert.getSource(), columnIndex, targetType);
            case UPDATE:
                SqlUpdate update = (SqlUpdate) query;
                final SqlNodeList sourceExpressionList = update.getSourceExpressionList();
                if (sourceExpressionList != null) {
                    return coerceColumnType(
                            sourceScope, sourceExpressionList, columnIndex, targetType);
                } else {
