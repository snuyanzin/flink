 ...
 *
 * <p>We make tool methods: {@link #coerceOperandType}, {@link #coerceColumnType}, {@link
 * #needToCast}, {@link #updateInferredType}, {@link #updateInferredColumnType} all overridable by
 * derived classes, you can define system specific type coercion logic.
 *
 ...
 *
 * <p>This class also defines the default implementation of the type widening strategies, see {@link
 * TypeCoercion} doc and methods: {@link #getTightestCommonType}, {@link #getWiderTypeFor}, {@link
 * #getWiderTypeForTwo}, {@link #getWiderTypeForDecimal}, {@link #commonTypeForBinaryComparison} for
 * the detail strategies.
 */
public abstract class AbstractTypeCoercion implements TypeCoercion {
    ...
    protected boolean coerceOperandsType(
            @Nullable SqlValidatorScope scope, SqlCall call, RelDataType commonType) {
    ...
    protected boolean coerceColumnType(
        ...
        if (node instanceof SqlCall) {
            ...
            if (node2.getOperator().kind == SqlKind.AS) {
                ...
                RelDataType targetType2 =
                        syncAttributes(validator.deriveType(scope, operand), targetType);
    ...
    /**
     * Sync the data type additional attributes before casting, i.e. nullability, charset,
     * collation.
     */
        ...
        if (fromType != null) {
            ...
            if (SqlTypeUtil.inCharOrBinaryFamilies(fromType)
                ...
                if (charset != null && SqlTypeUtil.inCharFamily(syncedType)) {
                    ...
                    syncedType =
                            factory.createTypeWithCharsetAndCollation(
                                    syncedType, charset, collation);
                }
            }
        }
        return syncedType;
    ...
    /**
     * Decide if a SqlNode should be casted to target type, derived class can override this
     * strategy.
     */
    ...
    private static SqlNode castTo(SqlNode node, RelDataType type) {
        return FlinkSqlOperatorTable.TRY_CAST.createCall(
                SqlParserPos.ZERO,
                node,
    ...
    /**
     * Update inferred row type for a query, i.e. SqlCall that returns struct type or SqlSelect.
     *
     * @param scope Validator scope
    ...
    protected void updateInferredColumnType(
            SqlValidatorScope scope, SqlNode query, int columnIndex, RelDataType desiredType) {
    ...
    public @Nullable RelDataType getTightestCommonType(
        ...
        if (type1.equals(type2)
                || (type1.isNullable() != type2.isNullable()
                        && factory.createTypeWithNullability(type1, type2.isNullable())
                                .equals(type2))) {
            return factory.createTypeWithNullability(
                    type1, type1.isNullable() || type2.isNullable());
        }
        ...
        if (type1.isStruct() && type2.isStruct()) {
            if (SqlTypeUtil.equalAsStructSansNullability(
                    factory, type1, type2, validator.getCatalogReader().nameMatcher())) {
     ...
     * other is not. For date + timestamp operands, use timestamp as common type, i.e.
     * Timestamp(2017-01-01 00:00 ...) &gt; Date(2018) evaluates to be false.
     */
    @Override
    ...
    /**
     * Case2: type widening. The main difference with {@link #getTightestCommonType} is that we
     * allow some precision loss when widening decimal to fractional, or promote fractional to
     * string type.
     */
    @Override
     ...
     * precision/scale exceeds system limitation, this rule will truncate the decimal type to the
     * max precision/scale. For decimal and fractional types, returns a decimal type which has the
     * higher precision of the two.
     *
     ...
     * #getWiderTypeForTwo} doesn't satisfy the associative law, i.e. (a op b) op c may not equal to
     * a op (b op c). This is only a problem for STRING type or nested STRING type in collection
     * type like ARRAY. Excluding these types, {@link #getWiderTypeForTwo} satisfies the associative
     * law. For instance, (DATE, INTEGER, VARCHAR) should have VARCHAR as the wider common type.
     */
    @Override
    ...
    /**
     * Checks if the types and families can have implicit type coercion. We will check the type one
