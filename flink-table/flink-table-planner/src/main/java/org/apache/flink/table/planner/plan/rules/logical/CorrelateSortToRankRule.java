 ...
 *
 * <p>{{{ LogicalProject(state=[$0], name=[$1]) +- LogicalCorrelate(correlation=[$cor0],
 * joinType=[inner], requiredColumns=[{0}]) :- LogicalAggregate(group=[{0}]) : +-
 * LogicalProject(state=[$1]) : +- LogicalTableScan(table=[[default_catalog, default_database,
 * cities]]) +- LogicalSort(sort0=[$1], dir0=[DESC-nulls-last], fetch=[3]) +-
 * LogicalProject(name=[$0], pop=[$2]) +- LogicalFilter(condition=[=($1, $cor0.state)]) +-
 * LogicalTableScan(table=[[default_catalog, default_database, cities]]) }}}
 *
 * <p>would be transformed to
 *
 * <p>{{{ LogicalProject(state=[$0], name=[$1]) +- LogicalProject(state=[$1], name=[$0], pop=[$2])
 * +- LogicalRank(rankType=[ROW_NUMBER], rankRange=[rankStart=1, rankEnd=3], partitionBy=[$1],
 * orderBy=[$2 DESC], select=[name=$0, state=$1, pop=$2]) +-
 * LogicalTableScan(table=[[default_catalog, default_database, cities]]) }}}
 *
