...
object UserDefinedFunctionUtils {
  ...
  def getEvalMethodSignatureOption(
    ...
    getEvalUserDefinedMethod(func, expectedTypes).map(
      method =>
  ...
  def getAggUserDefinedInputTypes(
    ...
    val accMethod =
      getAggFunctionUDIMethod(func, "accumulate", externalAccType, expectedTypes).getOrElse(
        throwValidationException(func.getClass.getCanonicalName, func, expectedTypes)
      )
...
//        expectedTypes.length))
  ...
  def getAccumulateMethodSignature(
      ...
      expectedTypes: Seq[LogicalType]): Option[Array[Class[_]]] = {
  ...
  def getEvalUserDefinedMethod(
      ...
      expectedTypes: Seq[LogicalType]): Option[Method] = {
  ...
  def getAggFunctionUDIMethod(
    ...
    val possibleSignatures =
      Seq(logicalTypesToInternalClasses(input), logicalTypesToExternalClasses(input))

    possibleSignatures.flatMap {
      signatures =>
        getUserDefinedMethod(
          function,
          methodName,
  ...
  def getUserDefinedMethod(
      function: UserDefinedFunction,
      methodName: String,
      signature: Seq[DataType]): Option[Method] = {
    getUserDefinedMethod(
      function,
      methodName,
  ...
  def getUserDefinedMethod(
      ...
      parameterDataTypeEquals: (LogicalType, DataType) => Boolean = parameterDataTypeEquals)
      : Option[Method] = {
    ...
    if (
      ...
      val nonObjectParameterMethods = found.filter {
        m => !m.getParameterTypes.contains(classOf[Object])
      }
  ...
  def getResultTypeClassOfScalarFunction(
    ...
    getEvalUserDefinedMethod(function, argTypes)
