...
class FlinkLogicalSortExchange(cluster: RelOptCluster, traitSet: RelTraitSet,
                               input: RelNode, distribution: RelDistribution, collation: RelCollation)
  extends LogicalSortExchange(cluster, traitSet, input, distribution, collation)
...
private class FlinkLogicalSortExchangeConverter(config: Config) extends ConverterRule(config) {
  ...
  override def convert(rel: RelNode): RelNode = {
    ...
    new FlinkLogicalSortExchange(input.getCluster, input.getTraitSet, input.getInput, input.getDistribution, input.getCollation)
  }
}
