...
object ScalarOperatorGens {
  ...
  def generateMap(
    ...
    val keyElements = elements.grouped(2)
      .map { case Seq(key, value) => (key, value) }.toSeq.groupBy(_._1).map(_._2.last).keys.toSeq
    ...
    val valueElements = elements.grouped(2)
      .map { case Seq(key, value) => (key, value) }.toSeq.groupBy(_._1).map(_._2.last).values.toSeq
