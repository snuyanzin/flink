...
object JsonGenerateUtils {
  ...
  def isOneOfFunctionDefinitions(rexNode: RexNode, expectedDefinitions: FunctionDefinition*): Boolean = {
    if (!rexNode.isInstanceOf[RexCall]) return false
    val call = rexNode.asInstanceOf[RexCall]
    val unwrapped = ShortcutUtils.unwrapFunctionDefinition(call)
    for (expected <- expectedDefinitions) {
      if (unwrapped eq expected) {
        return true
      }
      if (call.getOperator.isInstanceOf[SqlJsonArrayFunctionWrapper]) {
        return true
      } else if (call.getOperator.isInstanceOf[SqlJsonObjectFunctionWrapper]) {
        return true
      } else if (call.getOperator.isInstanceOf[SqlJsonQueryFunctionWrapper]) {
        return true
      } else if (call.getOperator.isInstanceOf[SqlJsonValueFunctionWrapper]) {
        return true
      }
    }
    false
