...
class RewriteIntersectAllRule
  ...
  override def onMatch(call: RelOptRuleCall): Unit = {
    ...
    val leftWithMarker = leftBuilder
      .push(left)
      .project(leftBuilder.fields(fields) ++ Seq(
        leftBuilder.alias(leftBuilder.literal(true), "vcol_left_marker"),
        leftBuilder.alias(leftBuilder.getRexBuilder.makeNullLiteral(boolType), "vcol_right_marker")
      ))
      .build()
    ...
    val rightWithMarker = rightBuilder
      ...
      .project(rightBuilder.fields(fields) ++ Seq(
        rightBuilder
          .alias(rightBuilder.getRexBuilder.makeNullLiteral(boolType), "vcol_left_marker"),
    ...
    builder
      ...
      .filter(builder.and(
        builder.call(GREATER_THAN_OR_EQUAL, builder.field("vcol_left_cnt"), builder.literal(1)),
        builder.call(GREATER_THAN_OR_EQUAL, builder.field("vcol_right_cnt"), builder.literal(1))
      ))
      .project(Seq(builder.call(
        IF,
        builder.call(GREATER_THAN, builder.field("vcol_left_cnt"), builder.field("vcol_right_cnt")),
