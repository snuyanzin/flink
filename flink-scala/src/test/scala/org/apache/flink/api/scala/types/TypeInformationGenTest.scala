...
class TypeInformationGenTest {
  ...
  def testGenericArrays(): Unit = {
    ...
    assertThat(
      getType(objectArray)
        ...
        .getComponentInfo)
      .isInstanceOf(classOf[PojoTypeInfo[_]])
  }
  ...
  def testTupleWithBasicTypes(): Unit = {
    ...
    assertThat(ti).isInstanceOf(classOf[TupleTypeInfoBase[_]])
    val tti = ti.asInstanceOf[TupleTypeInfoBase[_]]
    assertThat(tti.getTypeClass).isEqualTo(classOf[Tuple9[_, _, _, _, _, _, _, _, _]])
    for (i <- 0 until 9) {
      assertThat(tti.getTypeAt(i)).isInstanceOf(classOf[BasicTypeInfo[_]])
    }
  ...
  def testTupleWithTuples(): Unit = {
    ...
    assertThat(ti).isInstanceOf(classOf[TupleTypeInfoBase[_]])
    val tti = ti.asInstanceOf[TupleTypeInfoBase[_]]
    assertThat(tti.getTypeClass).isEqualTo(classOf[Tuple3[_, _, _]])
    ...
    assertThat(tti.getTypeAt(2).getTypeClass).isEqualTo(classOf[Tuple2[_, _]])
  ...
  def testTupleWithCustomType(): Unit = {
    ...
    assertThat(ti.getArity).isEqualTo(2)
    val tti = ti.asInstanceOf[TupleTypeInfoBase[_]]
    assertThat(tti.getTypeClass).isEqualTo(classOf[Tuple2[_, _]])
  ...
  def testTrait(): Unit = {
    ...
    assertThat(ti.isInstanceOf[GenericTypeInfo[TestTrait]]).isTrue
  }
  ...
  def testFieldAtStringRef(): Unit = {
    ...
    assertThat(caseClassInTupleTypeInfo.getTypeAt("1")).isEqualTo(caseClassTypeInfo)
    assertThat(caseClassInTupleTypeInfo.getTypeAt("_2")).isEqualTo(caseClassTypeInfo)
